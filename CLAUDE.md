...
## Reference material — `resette_cloud/` (TEMPORARY)

`resette_cloud/` is a **read-only copy of an unrelated prior project** (the
EDGI environmental data portal). It is here for one narrow purpose: to show
the Datasette plugin patterns this developer already knows.

**It will be deleted once patterns are extracted. Do not import from it, do
not add it to the package, do not let it influence the architecture.**

### What is worth extracting (roughly 3 patterns, not 200KB of code)

1. The `@hookimpl register_routes()` shape
2. `datasette.get_database(...).execute_write(...)` usage for POST handlers
3. Custom template + `metadata.json` wiring

### What must NOT be ported

The EDGI portal is a **multi-user, authenticated, publicly deployed web
application**. dev_road is a **single-user local tool on `localhost`**. Its
auth stack is not a feature here — it is unnecessary attack surface and
several thousand lines of maintenance burden:

- bcrypt authentication, sessions, login/register flows
- role-based access control, admin panels
- CSRF machinery beyond what Datasette already provides
- the upload engine, null-handling, data-quality analysis
- the trash/soft-delete system
- Docker, Fly.io, or any cloud deployment config

If a task seems to need user accounts or roles in dev_road, that is a signal
the task is wrong. There is one user, and they are sitting at the machine.

**Delete `resette_cloud/` after extraction**, and delete this section of
CLAUDE.md with it.
